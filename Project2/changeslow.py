...
def changeslow(value, change):
	if change == 0:
		return 0
	min_coins = maxint
	# for each value in the array, if it's less than the amount of change we want to make
	# then we call changeslow() recursively, subtracting that value from the goal value.
	# each iteration selects the minimum number between the two, such that after rebuilding
	# the solution, the smallest number of coins needed to make that amount of change is
